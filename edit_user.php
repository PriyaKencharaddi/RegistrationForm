...
// Get user ID from URL and ensure it's an integer
$id = intval($_GET['id'] ?? 0);
if (!$id) {
    die("Invalid user ID.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['resume_file']['name'])) {
        ...
        // Optionally delete old file
        if (!empty($user['resume_file']) && file_exists($uploadDir . $user['resume_file'])) {
            unlink($uploadDir . $user['resume_file']);
        }
    } else {
...
<div class="container mt-5">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="url" name="linkedin" class="form-control" value="<?= htmlspecialchars($user['linkedin']) ?>">
        </div>

        <div class="mb-3">
            <label class="form-label">GitHub</label>
            <input type="url" name="github" class="form-control" value="<?= htmlspecialchars($user['github']) ?>">
        </div>
